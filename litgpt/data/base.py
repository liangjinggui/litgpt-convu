...
class ConvINTSFTDataset(Dataset):
    """An in-memory dataset for supervised finetuning with `input_ids` and `labels`.
    ...
    def __getitem__(self, idx: int) -> Dict[str, Tensor]:
        example = self.data[idx]
        encoded_prompt_and_response = example["input_ids"]
        labels = example["labels"]
        return {"input_ids": encoded_prompt_and_response, "labels": labels}
    
    def preprocess_data(self, instances):
        
        assert self.transform is not None, "transform function is not defined"
        processed_instances = []
        for instance in instances:
            instance_features = self.transform(instance=instance, 
            ...
            processed_instances.append(instance_features)
